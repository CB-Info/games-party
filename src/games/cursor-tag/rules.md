...
- **Compte à rebours :** il dure `PREPARATION_COUNTDOWN_MS`, ne peut pas être annulé et envoie l'événement `preparationCountdown`. À 0, la manche démarre (section 4, étapes 1 à 6). Les joueurs qui n'étaient pas prêts démarrent sans souris capturée et peuvent cliquer à tout moment : ils voient l'invitation « Clique pour capturer ta souris » si la souris n'a encore jamais été capturée dans la partie, et sinon l'écran « Clique pour reprendre » (architecture 6.5).
...
- **Conséquences connues :**
  ...
  - quand une vue se perd, les autres interpolent un curseur sur deux ticks. Après un tick en retard, le budget s'est rechargé jusqu'à sa réserve pleine : un Chat peut alors parcourir deux réserves entre deux vues reçues, soit `2 × vitesse du Chat × MOVE_BUDGET_CAP_MS / 1000`. Au-delà de `TELEPORT_SNAP_DISTANCE`, les autres le verraient sauter au lieu de glisser : c'est le cas dès environ 1300 u/s pour le Coureur (1500 pour le Chat). Un test de garde le signale (`logic/movement.test.ts`), et cette constante de l'architecture devrait alors suivre. Le chiffre de 1750 u/s écrit au début de l'étape 4 supposait des ticks réguliers : une réserve, puis une seule recharge de 33 ms.
